...
namespace FluentMethods.UnitTests
{
    public partial class ILGeneratorFixture
    {
...
#endif
        public void BranchIfGreaterOrEqual()
        {
            ...
            il.BranchTo(elseLabel).IfGreaterOrEqual();
            il.LoadConst(false);
        ...
        [NUnit.Framework.Test]
#endif
        public void BranchIfGreaterOrEqualShortForm()
        {
            var method = new DynamicMethod("x", typeof(bool), new[] { typeof(int), typeof(int) });
            var il = method.GetILGenerator();
            var elseLabel = il.DefineLabel();
            ...
            il.Emit(OpCodes.Ldarg_0);
            il.Emit(OpCodes.Ldarg_1);
            il.BranchShortTo(elseLabel).IfGreaterOrEqual();
            il.LoadConst(false);
            il.Emit(OpCodes.Br, endLabel);
            il.MarkLabel(elseLabel);
            il.LoadConst(true);
