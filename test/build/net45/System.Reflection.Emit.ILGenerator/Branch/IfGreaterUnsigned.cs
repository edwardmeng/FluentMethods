...
namespace FluentMethods.UnitTests
{
    public partial class ILGeneratorFixture
    {
...
#endif
        public void BranchIfGreaterUnsigned()
        {
            ...
            il.BranchTo(elseLabel).IfGreaterUnsigned();
            il.LoadConst(false);
        ...
        [NUnit.Framework.Test]
#endif
        public void BranchIfGreaterUnsignedShortForm()
        {
            var method = new DynamicMethod("x", typeof(bool), new[] { typeof(uint), typeof(uint) });
            var il = method.GetILGenerator();
            var elseLabel = il.DefineLabel();
            ...
            il.Emit(OpCodes.Ldarg_0);
            il.Emit(OpCodes.Ldarg_1);
            il.BranchShortTo(elseLabel).IfGreaterUnsigned();
            il.LoadConst(false);
            il.Emit(OpCodes.Br, endLabel);
            il.MarkLabel(elseLabel);
            il.LoadConst(true);
