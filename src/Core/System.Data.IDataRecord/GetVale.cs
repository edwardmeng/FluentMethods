...
public static partial class Extensions
{
    ...
    public static T GetValue<T>(this IDataRecord record, string fieldName)
    {
        ...
        if (ordinal == -1)
        {
            throw new ArgumentException(string.Format(FluentMethods.Strings.FieldCannotBeFound, fieldName));
        }
...
#endif
                return default(T);
            throw new InvalidOperationException(string.Format(FluentMethods.Strings.CannotCastNullToValueType, fieldName, typeof(T)));
        }
        try
        {
