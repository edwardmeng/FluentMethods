...
public static partial class Extensions
{
    ...
    public static IQueryable<TSource> Range<TSource>(this IQueryable<TSource> source, int index, int count)
    {
        ...
        if (index < 0)
        {
            throw new ArgumentOutOfRangeException(nameof(index), FluentMethods.Strings.NeedNonNegNum);
        }
        if (count < 0)
        {
            throw new ArgumentOutOfRangeException(nameof(count), FluentMethods.Strings.NeedNonNegNum);
        }
        var query = source;
        if (index > 0)
        {
