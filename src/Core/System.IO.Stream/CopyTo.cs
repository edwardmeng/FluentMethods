...
public static partial class Extensions
{
    ...
    public static void CopyTo(this Stream source, Stream destination, int bufferSize)
    {
        ...
        if (bufferSize <= 0)
        {
            throw new ArgumentOutOfRangeException(nameof(bufferSize), FluentMethods.Strings.ArgumentOutOfRange_NeedPosNum);
        }
        if (!source.CanRead && !source.CanWrite)
        {
            throw new ObjectDisposedException(nameof(source), FluentMethods.Strings.ObjectDisposed_StreamClosed);
        }
        if (!destination.CanRead && !destination.CanWrite)
        {
            throw new ObjectDisposedException(nameof(destination), FluentMethods.Strings.ObjectDisposed_StreamClosed);
        }
        if (!source.CanRead)
        {
            throw new NotSupportedException(FluentMethods.Strings.NotSupported_UnreadableStream);
        }
