...
public static partial class Extensions
{
    ...
    public static async Task TraverseAsync<T>(this IEnumerable<T> source, Func<T, CancellationToken, Task<IEnumerable<T>>> childrenFactory, Func<T, CancellationToken, Task> action, CancellationToken token)
    {
        ...
        foreach (var v in source)
        {
            ...
            await TraverseAsync(await childrenFactory(v, token), childrenFactory, action, token);
        }
    }
    ...
    /// </exception>
    [DebuggerStepThrough]
    public static Task TraverseAsync<T>(this IEnumerable<T> source, Func<T, IEnumerable<T>> childrenFactory, Action<T> action, CancellationToken token)
    {
        return source.TraverseAsync((x, t) =>
        {
#if Net40
    ...
    /// </exception>
    [DebuggerStepThrough]
    public static Task TraverseAsync<T>(this IEnumerable<T> source, Func<T, IEnumerable<T>> childrenFactory, Action<T> action)
    {
        return source.TraverseAsync(childrenFactory, action, CancellationToken.None);
    }
}